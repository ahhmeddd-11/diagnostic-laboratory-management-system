...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('btn-print-with-header').addEventListener('click', async () => {
        ...
        window.location.replace(`/print?id=${currentPrintId}&header=true&pathologist=${incPatho}`);
        document.getElementById('print-modal').classList.add('hidden');
    ...
    document.getElementById('btn-print-no-header').addEventListener('click', async () => {
        const incPatho = document.getElementById('include-pathologist-sig').checked;
        window.location.replace(`/print?id=${currentPrintId}&header=false&pathologist=${incPatho}`);
        document.getElementById('print-modal').classList.add('hidden');
        ...
        tr.innerHTML = `
            ...
            <td>
                ${(currentUser.role === 'Admin' || currentUser.role === 'Operator') ? 
                    `<button class="btn-primary-outline" onclick="editPatient(${p.id})" style="padding: 4px 8px; font-size: 0.8rem; margin-right: 5px;">Edit</button>` 
                    : ''}
...
function openPatientModal(title, submitLabel, patientData = null, callback = null) {
    const isEdit = !!patientData;
    const isAdmin = currentUser && currentUser.role === 'Admin';
    const dateReadOnlyAttr = isAdmin ? '' : 'readonly style="opacity: 0.7; cursor: not-allowed;"';
    setModal(title, `
        <form id="form-patient">
            <div class="input-group">
                <label>Name</label>
                <input type="text" id="p-name" value="${isEdit ? patientData.name : ''}" required>
            </div>
            <div class="input-group">
                <label>Age</label>
                <input type="number" id="p-age" value="${isEdit ? patientData.age : ''}" required>
            </div>
            <div class="input-group">
                <label>Gender</label>
                <select id="p-gender">
            ...
            <div class="input-group">
                <label>Date</label>
                <input type="date" id="p-date" value="${isEdit ? patientData.date : new Date().toISOString().split('T')[0]}" ${dateReadOnlyAttr} required>
            </div>
            <div class="input-group">
                <label>Referred Doctor</label>
                <input type="text" id="p-doc" value="${isEdit ? (patientData.referred_doctor || '') : ''}">
            </div>
...
function openAddPatientModal() {
    openPatientModal('Add New Patient', 'Save Patient', null, async (data) => {
...
window.editPatient = (id) => {
    ...
    if (!patient) return;
    openPatientModal('Edit Patient', 'Update Patient', patient, async (data) => {
        await fetchAPI(`/patients/${id}`, { method: 'PUT', body: JSON.stringify(data) });
        await logActivity('DATA_ENTRY', `Updated patient: ${data.name} (ID: ${id})`);
        closeModal();
        loadPatients();
    });
};
        ...
        setModal(`Edit Report #${id}`, `
            <form id="form-edit-report">
                <div class="input-group">
                    ...
                    <input type="date" id="e-date" required value="${report.report_date ? report.report_date.split('T')[0] : report.created_at.split(' ')[0]}" ${currentUser && currentUser.role === 'Admin' ? '' : 'readonly style="opacity: 0.7; cursor: not-allowed;"'}>
                </div>
                <div class="input-group">
        ...
        setModal('Create Lab Report', `
            <form id="form-report">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    <div class="input-group">
                        <label>Report Date</label>
                        <input type="date" id="r-date" required value="${new Date().toISOString().split('T')[0]}" ${currentUser && currentUser.role === 'Admin' ? '' : 'readonly style="opacity: 0.7; cursor: not-allowed;"'}>
                    </div>
                    <div class="input-group">
    ...
    try {
        ...
        allUsers.forEach(u => {
            ...
            tr.innerHTML = `
                ...
                <td>
                    <button class="btn-primary-outline" onclick="editUser(${u.id})" style="padding: 4px 8px; font-size: 0.8rem; margin-right: 5px;">Edit</button>
...
function openUserModal(title, submitLabel, userData = null, callback = null) {
    const isEdit = !!userData;
    setModal(title, `
        <form id="form-user">
            <div class="input-group">
                <label>Name</label>
                <input type="text" id="u-name" value="${isEdit ? userData.name : ''}" required>
            </div>
            <div class="input-group">
                <label>Email</label>
                <input type="email" id="u-email" value="${isEdit ? userData.email : ''}" required>
            </div>
            <div class="input-group">
            ...
            <div class="input-group">
                ...
                <select id="u-role">
                    ...
                    <option value="Technician" ${isEdit && userData.role === 'Technician' ? 'selected' : ''}>Technician</option>
                    <option value="Operator" ${isEdit && userData.role === 'Operator' ? 'selected' : ''}>Operator</option>
                </select>
            </div>
            <button type="submit" class="btn-primary">${submitLabel}</button>
        </form>
    ...
    document.getElementById('form-user').addEventListener('submit', async (e) => {
        ...
        const passwordVal = document.getElementById('u-password').value;
        if (passwordVal) {
...
function openAddUserModal() {
    openUserModal('Create New User', 'Create User', null, async (data) => {
...
window.editUser = (id) => {
    ...
    if (!user) return;
    openUserModal('Edit User', 'Update User', user, async (data) => {
        await fetchAPI(`/users/${id}`, { method: 'PUT', body: JSON.stringify(data) });
        await logActivity('ADMIN', `Updated user: ${data.name} (ID: ${id})`);
        closeModal();
        loadUsers();
    });
};
